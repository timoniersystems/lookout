...
Access web UI at: http://localhost:3000 (or https://localhost:7443 via nginx when using Docker Compose)
...
CI enforces a minimum coverage threshold of 25% (warning only, non-blocking). Coverage reports are generated by the `coverage.yml` workflow and posted as PR comments.
...
- `gocritic` - Code quality diagnostics
